...
async function resolveEntryPoint(document, workspaceFolder, config, globalEntryPoint, bundledEntryPoint) {
    ...
    const configured = (config.get("runtime.entryPoint") ?? "").trim();
    const candidates = [];
    const seen = new Set();
    ...
    for (const candidate of candidates) {
        try {
            ...
            outputChannel.appendLine(`[resolveEntryPoint] using candidate ${candidate.path} (source=${candidate.source})`);
            return candidate;
        }
        catch (_error) {
    ...
    outputChannel.appendLine(`[resolveEntryPoint] failed. configured='${configured}', workspaceRoot='${workspaceRoot ?? "<none>"}' candidates=${candidates
        .map((c) => c.path)
        .join("; ")}`);
    return undefined;
}
...
function getBundledEntryPoint(context) {
    // Path to 'main.ts' INSIDE the 'dist' folder
    const bundledPath = path.join(context.extensionPath, "dist", "main.ts");
...
async function promptForEntryPoint(workspaceFolder, config) {
...
function registerRunner(context) {
    const runCommand = vscode.commands.registerCommand("datascript.runFile", async () => {
        ...
        if (!entryPoint) {
            vscode.window.showWarningMessage("Datascript entry point is not configured. Run command cancelled.");
